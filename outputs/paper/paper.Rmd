...
incomegraph <- incomeinfo %>% ggplot(aes(x = "", y = prop, fill = household_income_grouped)) + 
     ...
     geom_text(aes(label = paste0(round(prop), "%")), position = position_stack(vjust = 0.5), angle=0, size = 4.7) +
...
educationgraph <- educationinfo %>% ggplot(aes(x = "", y = prop, fill = education_grouped)) + 
     ...
     geom_text(aes(label = paste0(round(prop), "%")), position = position_stack(vjust = 0.5), angle=0, size = 4) +
     theme(text = element_text(size=13))
educationgraph
```
...
The dataset that we used contains many key features. For one, a lot of the variables contain an extensive list of options assuring us that the data represents the participant's view accurately. Despite the fact that we cleaned the dataset to make it simpler to view, we still believe that the detailed responses that the participants provided accurately reflect their views on the 2020 election. Another key feature is 
