...
That is 30,697,920 cells in total. We fit the logistic regressions for estimating candidate support in each cell. We used `function` from `package` to fit \@ref(eq:model).
...
```{r}
attach(clean_survey)
mod <- brm(binary ~ age + (1|education_grouped) + race_ethnicity + hispanic + household_income_grouped + state + extra_covid_worn_mask,
           data = clean_survey,
           family = bernoulli(),
           iter = 4000,
...
```{r}
posterior_prob <- posterior_linpred(mod, transform = TRUE, newdata = post_strat_cellcount, allow_new_levels = TRUE)
poststrat_prob <- posterior_prob %*% post_strat_cellcount$n / sum(poststrat$n)
...
res_state <- mod %>%
  ...
  rename(president_predict = .prediction) %>%  
  mutate(president_predict_prop = president_predict*prop) %>% 
