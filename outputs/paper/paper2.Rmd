---
title: "Biden Forcested To Win the Election"
subtitle: "Based on US Demographics of Age, Education, Household Income and Race"
...
The first few rows of the cleaned dataset can be viewed in Table 1 below:
### Table 1 
...
In Figure 1 below, we will examine the variable 'household_income' and we can see that our data is largely skewed towards households with income less than $50,000:
### Figure 1 
```{r, tidy =FALSE, echo =FALSE, warning= FALSE}
...
We can see a similar skewed distribution with variable 'race' in Figure 2 below:
### Figure 2
```{r, tidy =FALSE, echo =FALSE, warning= FALSE}
# Setup data for the pie chart and mutate a variable that details proportions for various responses. 
...
Lastly, by examining the variable age in Figure 3 below, we can see how very little the population 76+ is represented in our dataset. 
### Figure 3
```{r, tidy =FALSE, echo =FALSE, warning= FALSE}
# Setup data for the pie chart and mutate a variable that details proportions for various responses. 
...
The logistic regression with levels allows us to compare levels for each variable at a comparable level. The estimates for $\beta_{age}$ compare other groups to age from 18 to 35; the estimates for $\beta_{race}$ compares different levels to American Indian or Alaska native, and the estimates for $\beta_{income}$ compare other income levels to income level from 100k to 125k. For optimal display purposes, we do not state the estimated logistic regression formula. You can interpret this as subbing each$\beta$ in \@ref(eq:logistic).

Using $\alpha = 0.05, H_0: \hat{\beta} = 0, \hat{\beta} \neq0$, we can determine if a certain variable (or its levels) have statistically significant influence towards the response variable. The p-value indicates the probability of having a more or equally extreme observation than the observed one, given that the null hypothesis is true. Naturally, a smaller p-value makes our estimate more statistically significant. We define “small” by setting $\alpha = 0.05$. A variable is said to be significantly influential to the response variable if it has a p-value smaller than 0.05. Therefore we can reject $H_0$ and $\beta_1 \neq 0$. Recall that we are finding out whether a respondent would vote for Trump in the presidential election. 

Observing table 1, the p-value indicates weak evidence that being white is related to one’s voting intentions(0.68 > 0.05). Having a family income of greater than 150k a year influences whether the respondent shall vote for Trump, but not as much compared to the other variables. The following levels have p-value very close to 0:
Age > 76;
Being Asian or Pacific Islander;
Being NEC;
Having a family income smaller than 25k;
Having a family income between 25k and 50k;
Having a family income between 75k and 100k.
This means that these variables have a statistically significant relationship with whether the respondent shall vote for Trump.

Among these variables, we notice an obvious pattern among racial groups, where respondents from a minor race are much less likely to vote for Trump. This makes sense because of the history behind the Republican party and the `xenophobic` image Trump makes. Specifically, he made many discriminatory comments against the minorities, and his travel bans from third world countries emphasize this image once again. 

Younger respondents(18 to 35) are less likely to vote for Trump than all age groups, while the oldest age group(more aged than 76) goes furthest in intending to vote for Trump. This is reasonable since older people who can potentially be more patriotic in a traditional way might harmonize Trump’s political opinions better than the young. The ban of TikTok certainly does not make his more likable among youth either.  

Respondents with less income have less incentive to vote for Trump, while this incentive roughly increases across increased income levels. As a capitalist himself, it is not surprising that Trump does not gain generous support among groups more towards the working class.
...
# label the data as it is a dta
acs <- labelled::to_factor(acs)
#create a tibble that includes counts and proportions based on groups
incomeinfoacs <- acs %>% group_by(household_income_grouped) %>% summarise(n=n()) %>% mutate(prop = 100*(n/sum(n)))
#Graph this tibble in a pie graph
...
#Generate a table that displays the first couple responses from the cell count that was generated for weighting 
...
```{r include=FALSE}
#Get general percentage of people voting for Trump by Race
aggregate(tbdata[8], list(tbdata$race_ethnicity), mean)
#get overal percentage of people voting for Trump
mean(tbdata$binary)  
```
...
### Preamble- Setting Up Libraries and Clean Data 
...
# Weighting code that did not run 
```{r echo=TRUE, results = "hide", warning= FALSE}
...
- Jared E. Knowles and Carl Frederick (2020). merTools: Tools for Analyzing Mixed Effect Regression Models. R package version 0.5.2.
...
- Monica Alexander. (2019, September 07). Analyzing name changes after marriage using a non-representative survey. Retrieved from https://www.monicaalexander.com/posts/2019-08-07-mrp/
- The New York Times. (n.d.). 2016 Presidential Election Results: Donald J. Trump Wins. Retrieved from https://www.nytimes.com/elections/2016/results/president
- US Census Bureau. (2020, October 16). Population. Retrieved from https://www.census.gov/topics/population.html
- NCSL. (2020). The Electoral College. Retrieved from https://www.ncsl.org/research/elections-and-campaigns/the-electoral-college.aspx
- Financial Times. (2020, October 30). Biden vs Trump: US presidential election poll tracker. Retrieved from https://ig.ft.com/us-election-2020/
...
- Amy Sorkin, John Cassidy, & Jonathan Blitzer. (2018). Donald Trump Compounds His "Shithole Countries" Bigotry with Lies. Retrieved from https://www.newyorker.com/news/daily-comment/donald-trump-compounds-his-shithole-countries-bigotry-with-lies
- Philip Bump. (2020, September 30). Analysis | Donald Trump doesn't really care about the white supremacist threat, and you're not going to make him. Retrieved from https://www.washingtonpost.com/politics/2020/09/30/donald-trump-doesnt-really-care-about-white-supremacy-youre-not-going-make-him/
- Kevin Liptak, & Kristen Holmes. (2020, July 01). Trump calls Black Lives Matter a 'symbol of hate' as he digs in on race. Retrieved from https://www.cnn.com/2020/07/01/politics/donald-trump-black-lives-matter-confederate-race/index.html
- Democracy Fund, & UCLA. (2020). Nation Scape Data Set. Retrieved from https://www.voterstudygroup.org/publication/nationscape-data-set
- US Census Bureau. (2018). American Community Survey 2018. Retrieved from https://usa.ipums.org/usa/index.shtml
